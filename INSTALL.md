...
```bash
# Windows (PowerShell)
Copy-Item -Path "N:\编程备份\4.0团队\sterilizer-team\agents\*.md" -Destination "$env:USERPROFILE\.claude\agents\" -Force

# Windows (Git Bash)
cp -r "N:/编程备份/4.0团队/sterilizer-team/agents/"*.md ~/.claude/agents/
...
```bash
# Windows (PowerShell)
Copy-Item -Path "N:\编程备份\4.0团队\sterilizer-team\skills\sterilizer-coordinator" -Destination "$env:USERPROFILE\.claude\skills\" -Recurse -Force

# Windows (Git Bash)
cp -r "N:/编程备份/4.0团队/sterilizer-team/skills/sterilizer-coordinator" ~/.claude/skills/
...
```bash
# Windows (PowerShell)
Remove-Item "$env:USERPROFILE\.claude\agents\sterilizer-*.md" -Force
Remove-Item "$env:USERPROFILE\.claude\skills\sterilizer-coordinator" -Recurse -Force
